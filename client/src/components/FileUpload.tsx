  ...
  const uploadMutation = useMutation({
    mutationFn: async (file: File) => {
      const attemptUpload = async (attempt: number): Promise<any> => {
        ...
        try {
          console.log(`[FileUpload] Upload attempt ${attempt}: ${file.name} (${(file.size/1024).toFixed(1)}KB)`);
      ...
      try {
        ...
        console.warn(`[FileUpload] First upload attempt failed: ${firstErr.message}, retrying in 2s...`);
        await new Promise(r => setTimeout(r, 2000));
        try {
          return await attemptUpload(2);
        } catch (secondErr: any) {
          console.warn(`[FileUpload] Second upload attempt failed: ${secondErr.message}, retrying in 4s...`);
          await new Promise(r => setTimeout(r, 4000));
          return await attemptUpload(3);
        }
      }
    },
      ...
      if (selectedFile) {
        try {
          ...
          if (isPdf) {
            ...
            const extractPdfWithLib = async (): Promise<{ text: string; pages: number; images: string[] }> => {
              const pdfjsLib = await import("pdfjs-dist");
              pdfjsLib.GlobalWorkerOptions.workerSrc = "";
              const pdf = await pdfjsLib.getDocument({ data: pdfData, disableWorker: true } as any).promise;
              const pages: string[] = [];
              const pageImages: string[] = [];
              ...
              return { text: pages.join("\n\n"), pages: pdf.numPages, images: pageImages };
            };
            
            try {
              const result = await extractPdfWithLib();
              console.log(`[FileUpload] PDF extracted client-side (no worker): ${result.pages} pages, ${result.text.length} chars`);
              onFileAnalyzed({
                fileName: file.name,
                fileType: "application/pdf",
                content: result.text.slice(0, 50000),
                metadata: { clientSideRead: true, pdfPages: result.pages, pdfPageImages: result.images },
              }, file.name);
              setSelectedFile(null);
              setUploadStatus("idle");
              resetInputs();
              ...
              console.error("[FileUpload] PDF client extraction failed, trying background save...", pdfErr);
              try {
                const saveFormData = new FormData();
                saveFormData.append("file", file);
                const saveResp = await fetch("/api/files/upload", {
                  method: "POST",
                  credentials: "include",
                  body: saveFormData,
                });
                if (saveResp.ok) {
                  ...
                  console.log("[FileUpload] Background save succeeded after client extraction failure");
                  const analysis = saveData.analysis || {
                    fileName: file.name,
                    fileType: "application/pdf",
                    content: saveData.file?.description || `[PDF sauvegardé: ${file.name}]`,
                    metadata: {},
                  };
                  onFileAnalyzed(analysis, file.name);
                  setSelectedFile(null);
                  setUploadStatus("idle");
                  resetInputs();
                  return;
                ...
                console.warn("[FileUpload] Background save also failed:", bgErr);
              }
              ...
              onFileAnalyzed({
                fileName: file.name,
                fileType: "application/pdf",
                content: `[PDF: ${file.name} (${(file.size / 1024).toFixed(1)} Ko) — extraction texte échouée, fichier binaire joint en base64]`,
                metadata: { clientSideRead: true, pdfBase64Full: fullBase64, uploadError: error.message },
              }, file.name);
              setSelectedFile(null);
              setUploadStatus("idle");
              resetInputs();
