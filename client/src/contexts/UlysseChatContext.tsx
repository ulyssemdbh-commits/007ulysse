...
export function UlysseChatProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const syncToLatest = () => {
      ...
      if (latest && latest.id !== sharedConvId) {
        console.log(`[ChatSync] Switching to latest conversation #${latest.id} (was #${sharedConvId})`);
        setSharedConvId(latest.id);
        setSharedConversationId(latest.id);
        queryClient.invalidateQueries({ queryKey: ["/api/conversations", latest.id] });
      }
    };
    window.addEventListener("ulysse:sync-reconnected", syncToLatest);
    return () => window.removeEventListener("ulysse:sync-reconnected", syncToLatest);
  }, [user, conversations, sharedConvId, queryClient]);
