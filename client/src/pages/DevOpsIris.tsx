        ...
        <div className="relative max-w-7xl mx-auto px-6 py-6 space-y-6">
          ...
          >
            ...
            <div className="flex gap-2">
              {isAdmin && (
                <Button variant="outline" size="sm" className="rounded-xl" onClick={() => runCleanup(true)} disabled={cleanupLoading} data-testid="button-scan-orphans-iris">
                  {cleanupLoading ? <Loader2 className="h-4 w-4 mr-2 animate-spin" /> : <Trash2 className="h-4 w-4 mr-2" />}
                  Orphelins
                </Button>
              )}
              ...
              <div className="flex items-center justify-between mb-2">
                ...
                <div className="flex gap-2">
                  {cleanupResult.orphaned?.length > 0 && !cleanupResult.deleted?.length && (
                    <Button size="sm" variant="destructive" className="h-7 text-xs rounded-lg" onClick={() => runCleanup(false)} disabled={cleanupLoading} data-testid="button-delete-orphans-iris">
                      Supprimer {cleanupResult.orphaned.length} orphelin(s)
                    </Button>
                  )}
                  <Button size="sm" variant="ghost" className="h-7 text-xs" onClick={() => setCleanupResult(null)} data-testid="button-close-cleanup-iris">
                    <X className="w-3 h-3" />
                  </Button>
                </div>
              </div>
