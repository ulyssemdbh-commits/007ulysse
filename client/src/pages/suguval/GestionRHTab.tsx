            ...
            <div className="p-3 sm:p-4">
                <div className="flex items-start sm:items-center justify-between gap-2">
                    <div className="flex items-center gap-2 sm:gap-3 min-w-0 flex-1">
                        <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-gradient-to-br from-orange-500 to-red-500 flex items-center justify-center font-bold text-xs sm:text-sm flex-shrink-0">
                            {e.firstName[0]}{e.lastName[0]}
                        </div>
                        <div className="min-w-0 flex-1">
                            <p className="font-medium text-sm sm:text-base truncate">{e.firstName} {e.lastName}</p>
                            <p className={`text-[11px] sm:text-xs ${dk ? "text-white/40" : "text-slate-400"} truncate`}>
                                {e.role} • <span className={contractColor[e.contractType] || (dk ? "text-white/60" : "text-slate-600")}>{e.contractType}</span>
                                <span className="hidden sm:inline"> • Depuis {fmtDate(e.startDate)}{e.contractType === "CDD" && e.endDate ? ` → ${fmtDate(e.endDate)}` : ""}</span>
                            </p>
                            <p className={`text-[10px] sm:hidden ${dk ? "text-white/30" : "text-slate-400"}`}>Depuis {fmtDate(e.startDate)}{e.contractType === "CDD" && e.endDate ? ` → ${fmtDate(e.endDate)}` : ""}</p>
                        </div>
                    </div>
                    <div className="flex items-center gap-2 sm:gap-4 flex-shrink-0">
                        <div className="text-right">
                            <p className="font-mono font-bold text-sm sm:text-base">{fmt(e.monthlySalary ?? 0)}<span className={`text-[10px] sm:text-xs ${dk ? "text-white/40" : "text-slate-400"}`}>/mois</span></p>
                            {e.weeklyHours && <p className={`text-[10px] sm:text-xs ${dk ? "text-white/50" : "text-slate-500"}`}>{e.weeklyHours}h/sem</p>}
                        </div>
        ...
        <div className="space-y-6">
            ...
            {filteredPayrolls.length > 0 && (
                <div className={`${dk ? "bg-white/5" : "bg-white"} border ${dk ? "border-white/10" : "border-slate-200"} rounded-2xl p-3 sm:p-5`}>
                    <h3 className={`font-semibold text-sm sm:text-base ${dk ? "text-white" : "text-slate-800"} mb-2 sm:mb-3 flex items-center gap-2`}>
                        <Calculator className="w-4 h-4 sm:w-5 sm:h-5 text-blue-400" /> Synthèse financière
                    </h3>
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-2 sm:gap-3">
                        <div className={`${dk ? "bg-white/5" : "bg-slate-50"} rounded-xl p-2 sm:p-3 text-center`}>
                            <p className={`text-[9px] sm:text-[10px] ${dk ? "text-white/40" : "text-slate-400"} mb-0.5 sm:mb-1`}>Coût employeur</p>
                            <p className="text-sm sm:text-base font-mono font-bold text-orange-400">{fmt(totalCoutEmployeur)}</p>
                            <p className={`text-[8px] sm:text-[9px] mt-0.5 ${dk ? "text-white/25" : "text-slate-300"}`}>brut + charges pat.{hasGlobalEstimate ? " ~" : ""}</p>
                        </div>
                        <div className={`${dk ? "bg-white/5" : "bg-slate-50"} rounded-xl p-2 sm:p-3 text-center`}>
                            <p className={`text-[9px] sm:text-[10px] ${dk ? "text-white/40" : "text-slate-400"} mb-0.5 sm:mb-1`}>Total brut</p>
                            <p className="text-sm sm:text-base font-mono font-bold text-orange-400">{fmt(totalPayrollGross)}</p>
                            <p className={`text-[8px] sm:text-[9px] mt-0.5 ${dk ? "text-white/25" : "text-slate-300"}`}>moy/fiche: {fmt(avgGrossPerEmp)}</p>
                        </div>
                        <div className={`${dk ? "bg-white/5" : "bg-slate-50"} rounded-xl p-2 sm:p-3 text-center`}>
                            <p className={`text-[9px] sm:text-[10px] ${dk ? "text-white/40" : "text-slate-400"} mb-0.5 sm:mb-1`}>Total net versé</p>
                            <p className="text-sm sm:text-base font-mono font-bold text-green-400">{fmt(totalPayrollNet)}</p>
                            <p className={`text-[8px] sm:text-[9px] mt-0.5 ${dk ? "text-white/25" : "text-slate-300"}`}>écart: {fmt(totalPayrollGross - totalPayrollNet)}</p>
                        </div>
                        <div className={`${dk ? "bg-white/5" : "bg-slate-50"} rounded-xl p-2 sm:p-3 text-center`}>
                            <p className={`text-[9px] sm:text-[10px] ${dk ? "text-white/40" : "text-slate-400"} mb-0.5 sm:mb-1`}>Charges salariales</p>
                            <p className="text-sm sm:text-base font-mono font-bold text-red-400">{fmt(totalPayrollChargesSal)}</p>
                            <p className={`text-[8px] sm:text-[9px] mt-0.5 ${dk ? "text-white/25" : "text-slate-300"}`}>taux: {globalChargeRate}% du brut</p>
                        </div>
                        <div className={`col-span-2 sm:col-span-1 ${dk ? "bg-white/5" : "bg-slate-50"} rounded-xl p-2 sm:p-3 text-center`}>
                            <p className={`text-[9px] sm:text-[10px] ${dk ? "text-white/40" : "text-slate-400"} mb-0.5 sm:mb-1`}>Rétention nette</p>
                            <p className="text-sm sm:text-base font-mono font-bold text-blue-400">{totalPayrollGross > 0 ? (totalPayrollNet / totalPayrollGross * 100).toFixed(1) : "0"}%</p>
                            <p className={`text-[8px] sm:text-[9px] mt-0.5 ${dk ? "text-white/25" : "text-slate-300"}`}>net / brut</p>
                        </div>
                    </div>
                </div>
            )}
            ...
            {activeEmps.length > 0 && (
                <div className={`${dk ? "bg-white/5" : "bg-white"} border ${dk ? "border-white/10" : "border-slate-200"} rounded-2xl p-3 sm:p-5`}>
                    <h3 className={`font-semibold text-sm sm:text-base ${dk ? "text-white" : "text-slate-800"} mb-2 sm:mb-3 flex items-center gap-2`}><Users className="w-4 h-4 sm:w-5 sm:h-5 text-orange-400" /> Répartition des contrats</h3>
            ...
            {/* Search + Filters */}
            <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-5 gap-2 items-center">
                <div className={`col-span-2 flex items-center gap-2 ${dk ? "bg-white/5" : "bg-white"} border ${dk ? "border-white/10" : "border-slate-200"} rounded-lg px-3 py-2`}>
                    ...
                    <input value={rhSearch} onChange={e => setRhSearch(e.target.value)} placeholder="Rechercher nom, poste..." className="bg-transparent w-full text-sm focus:outline-none" />
                </div>
                ...
                <button onClick={exportRhCSV} className={`col-span-2 sm:col-span-1 px-3 py-2 text-sm rounded-lg bg-gradient-to-r from-emerald-500 to-teal-500 ${dk ? "text-white" : "text-slate-800"} whitespace-nowrap`}>Export CSV</button>
            </div>
            ...
            <Card title={`Fiches de Paie (${filteredPayrolls.length})`} icon={DollarSign}
                action={!restricted ? <div className="flex flex-wrap gap-1.5 sm:gap-2 justify-end">
                    ...
                    <button onClick={() => payrollPdfRef.current?.click()} className={`${btnPrimary} !px-2 sm:!px-4 !py-1.5 sm:!py-2 text-[11px] sm:text-sm`} disabled={importingPayroll} data-testid="button-import-payroll-pdf" title="Importer un bulletin de paie PDF">
                        {importingPayroll ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Upload className="w-3.5 h-3.5" />}
                        {importingPayroll ? (importProgress || "...") : "PDF"}
                    </button>
                    <button onClick={handleReparseAll} className={`px-2 sm:px-3 py-1.5 sm:py-2 text-[11px] sm:text-xs rounded-lg ${dk ? "bg-amber-500/20 text-amber-400 hover:bg-amber-500/30" : "bg-amber-100 text-amber-700 hover:bg-amber-200"} transition flex items-center gap-1`} disabled={reparsing} data-testid="button-reparse-payroll" title="Re-parser tous les bulletins PDF existants">
                        {reparsing ? <Loader2 className="w-3 h-3 animate-spin" /> : <RefreshCw className="w-3 h-3" />}
                        <span className="hidden sm:inline">{reparsing ? "Re-parse..." : "Re-parser"}</span>
                    </button>
                    <button onClick={() => setShowPayrollForm(true)} className={`${btnPrimary} !px-2 sm:!px-4 !py-1.5 sm:!py-2 text-[11px] sm:text-sm`} data-testid="button-add-payroll"><Plus className="w-3.5 h-3.5" /> <span className="sm:hidden">+</span><span className="hidden sm:inline">Ajouter Fiche</span></button>
                </div> : undefined}>
                {filteredPayrolls.length === 0 ? (
