...
function formatEntry(entry: LogEntry): string {
  const { timestamp, level, service, message, data, userId, requestId, duration } = entry;
  
  const meta: string[] = [];
  if (userId) meta.push(`userId=${userId}`);
  if (requestId) meta.push(`reqId=${requestId}`);
  if (duration !== undefined) meta.push(`duration=${duration}ms`);
  
  const metaStr = meta.length > 0 ? ` [${meta.join(" ")}]` : "";
  const dataStr = data ? ` ${JSON.stringify(data)}` : "";
  
  return `${timestamp} [${level.toUpperCase()}] [${service}]${metaStr} ${message}${dataStr}`;
}
...
export const logger = {
  ...
  homework: createLogger("Homework"),
  mars: createLogger("MARS"),
  email: createLogger("Email"),
  sync: createLogger("Sync"),
  scheduler: createLogger("Scheduler"),
  sports: createLogger("Sports"),
  sugu: createLogger("SUGU"),
