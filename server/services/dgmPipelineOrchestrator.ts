...
const REPO_APP_MAP: Record<string, string> = {
  ...
  "mdbhdev": "mdbhdev",
  "ulysseproject": "ulysse",
  "007ulysse": "007ulysse",
  "007ulysse-test": "007ulysse-dev",
};
...
async function notifyPipelineResult(
  sessionId: number,
  taskId: number,
  finalStatus: string,
  config: PipelineConfig,
  ...
  try {
    ...
    const [task] = await db.select().from(dgmTasks).where(eq(dgmTasks.id, taskId));
    const isSuccess = finalStatus === "pipeline_complete" || finalStatus === "pr_created_awaiting_merge";
    const isFailure = finalStatus.includes("failed") || finalStatus.includes("error") || finalStatus.includes("blocked");

    const statusEmoji = isSuccess ? "\u2705" : isFailure ? "\u274C" : "\u26A0\uFE0F";
    const type = isSuccess ? "success" as const : isFailure ? "error" as const : "warning" as const;

    const durationStr = metrics.totalDurationMs >= 60_000
      ? `${Math.floor(metrics.totalDurationMs / 60_000)}m ${Math.round((metrics.totalDurationMs % 60_000) / 1000)}s`
      : `${Math.round(metrics.totalDurationMs / 1000)}s`;

    const fields = [
      { name: "Status", value: `${statusEmoji} ${finalStatus}`, inline: true },
      { name: "Repo", value: `${config.owner}/${config.repo}`, inline: true },
      { name: "Duration", value: durationStr, inline: true },
    ];
    if (metrics.retryCount > 0) fields.push({ name: "Retries", value: `${metrics.retryCount}`, inline: true });
    if (prUrl) fields.push({ name: "PR", value: prUrl, inline: false });

    await discordService.sendNotification({
      title: `DGM Pipeline ${isSuccess ? "Complete" : "Alert"} - Task #${taskId}`,
      message: task?.title || "Unknown task",
      type,
      fields,
    });
    console.log(`${LOG} Discord notification sent for task ${taskId}: ${finalStatus}`);
  } catch (e: any) {
    console.warn(`${LOG} Discord notification failed: ${e.message}`);
  }
}
...
export async function runRollback(
  ...
  try {
    const [task] = await db.select().from(dgmTasks).where(eq(dgmTasks.id, taskId));
    if (!task?.prNumber) {
      return { success: false, error: "No PR number — cannot rollback" };
    }

    await db.update(dgmTasks).set({ pipelineStage: "failed", status: "failed", error: `Rollback: ${reason}` }).where(eq(dgmTasks.id, taskId));

    const ghService = await import("./githubService");

    const prData = await ghService.githubApi(`/repos/${config.owner}/${config.repo}/pulls/${task.prNumber}`);
    const mergeCommitSha = (prData as any)?.merge_commit_sha;

    let revertResult: any = { reason, prNumber: task.prNumber };

    if (mergeCommitSha) {
      try {
        const revertBranch = `dgm/revert-task-${taskId}-${Date.now().toString(36)}`;
        const mainBranch = config.branch || "main";
        const branchInfo = await ghService.getBranch(config.owner, config.repo, mainBranch);
        const headSha = (branchInfo as any)?.commit?.sha;

        if (headSha) {
          await ghService.createBranch(config.owner, config.repo, revertBranch, headSha);

          const revertPr = await ghService.createPullRequest(
            config.owner, config.repo,
            `[DGM-ROLLBACK] Revert task #${taskId}: ${task.title}`,
            `## Rollback automatique DGM\n\n**Raison:** ${reason}\n**PR originale:** #${task.prNumber}\n**Merge commit:** ${mergeCommitSha}\n\n---\n*Rollback automatique par DGM Pipeline V2*`,
            revertBranch, mainBranch
          );
          revertResult.revertPrNumber = (revertPr as any)?.number;
          revertResult.revertPrUrl = (revertPr as any)?.html_url;
          console.log(`${LOG} Revert PR created: ${revertResult.revertPrUrl}`);
        }
      } catch (revertErr: any) {
        console.error(`${LOG} Git revert failed, falling back to app restart: ${revertErr.message}`);
        revertResult.revertError = revertErr.message;
      }
    }
    ...
    if (config.appName) {
      try {
        const { sshService } = await import("./sshService");
        const restartResult = await sshService.updateApp(config.appName, config.branch || "main");
        revertResult.appRestarted = true;
        revertResult.restartResult = restartResult;
        console.log(`${LOG} App ${config.appName} restarted after rollback`);
      } catch (restartErr: any) {
        revertResult.restartError = restartErr.message;
        console.error(`${LOG} App restart failed during rollback: ${restartErr.message}`);
      }
    }

    const durationMs = Date.now() - start;
    logPipelineRun(sessionId, taskId, "rollback", "completed", { reason, prNumber: task.prNumber }, revertResult, durationMs);

    await notifyPipelineResult(sessionId, taskId, "rollback_executed", config, {
      totalDurationMs: durationMs, stageDurations: {}, retryCount: 0, filesProcessed: 0, cacheHits: 0, cacheMisses: 0,
    });

    console.log(`${LOG} Rollback completed task ${taskId} [${durationMs}ms]`);
    return { success: true, data: revertResult, durationMs };
  } catch (err: any) {
    const durationMs = Date.now() - start;
    logPipelineRun(sessionId, taskId, "rollback", "failed", { reason }, null, durationMs, err.message);
    return { success: false, error: err.message, durationMs };
  }
}
...
export async function runFullPipeline(sessionId: number, taskId: number, config: PipelineConfig, patchFiles: { path: string; content: string }[], commitMessage: string): Promise<{ stages: Record<string, StageResult>; finalStatus: string; metrics: PipelineMetrics }> {
  ...
  if (riskScore >= DGM_GOVERNANCE.breakageProtection.maxRiskScoreAutoMerge) {
    ...
    metrics.totalDurationMs = Date.now() - pipelineStart;
    await notifyPipelineResult(sessionId, taskId, "blocked_high_risk", config, metrics);
  ...
  if (finalPatchFiles.length === 0) {
    ...
    if (!devPatch.stageResult.success || devPatch.patchFiles.length === 0) {
      metrics.totalDurationMs = Date.now() - pipelineStart;
      await notifyPipelineResult(sessionId, taskId, "failed_at_dev_patch", config, metrics);
  ...
  while (!reviewPassed && retryCount <= maxRetries) {
    ...
    if (!review.success) {
      metrics.totalDurationMs = Date.now() - pipelineStart;
      await notifyPipelineResult(sessionId, taskId, "failed_at_review", config, metrics);
  ...
  if (!pr.success) {
    metrics.totalDurationMs = Date.now() - pipelineStart;
    await notifyPipelineResult(sessionId, taskId, "failed_at_pr", config, metrics);
  ...
  if (config.autoMerge) {
    ...
    if (!merge.success) {
      ...
      metrics.totalDurationMs = Date.now() - pipelineStart;
      await notifyPipelineResult(sessionId, taskId, "failed_at_merge", config, metrics);
    ...
    if (config.autoDeploy && config.appName) {
      ...
      if (!monitor.success && DGM_GOVERNANCE.breakageProtection.rollbackOnHealthFailure) {
        console.log(`${LOG} Post-deploy health FAILED — executing rollback`);
        const rollback = await runRollback(sessionId, taskId, config, "Post-deploy health check failed");
        stages.rollback = rollback;
        metrics.stageDurations.rollback = rollback.durationMs || 0;
        metrics.totalDurationMs = Date.now() - pipelineStart;
        const prUrl = stages.pr_creation?.data?.prUrl;
        await notifyPipelineResult(sessionId, taskId, "deployed_unhealthy_rolled_back", config, metrics, prUrl);
        return { stages, finalStatus: "deployed_unhealthy_rolled_back", metrics };
      }
    }
  }
