...
const COBA_TOOLS: OpenAI.Chat.Completions.ChatCompletionTool[] = [
  ...
  },
  {
    ...
    function: {
      name: "coba_files",
      description: "Lister les fichiers du restaurant, obtenir un fichier par ID, ou les statistiques. Chaque fichier retourné inclut un downloadUrl pour le téléchargement direct.",
      parameters: {
        type: "object",
        properties: {
          action: { type: "string", enum: ["list", "get", "stats"], description: "list = lister les fichiers (avec downloadUrl), get = obtenir un fichier par ID, stats = statistiques par catégorie" },
          category: { type: "string", description: "Filtrer par catégorie (releve_bancaire, bulletin_paie, facture, autre)" },
          file_id: { type: "integer", description: "ID du fichier (pour action=get)" },
        },
        required: ["action"]
      }
    }
...
async function executeToolCall(tenantId: string, toolName: string, args: any): Promise<string> {
  try {
    ...
    switch (toolName) {
      ...
      case "coba_files": {
        const { action, category, file_id } = args;
        if (action === "list") {
          const files = await cobaBusinessService.listFiles(tenantId, { category });
          const enriched = files.map((f: Record<string, unknown>) => ({
            ...f,
            downloadUrl: `/api/coba/business/files/${tenantId}/${f.id}/download`,
          }));
          return JSON.stringify({ data: enriched });
        }
        if (action === "get") {
          if (!file_id) return JSON.stringify({ error: "file_id requis pour action=get" });
          const file = await cobaBusinessService.getFile(tenantId, file_id);
          if (!file) return JSON.stringify({ error: "Fichier introuvable" });
          return JSON.stringify({
            data: { ...file, downloadUrl: `/api/coba/business/files/${tenantId}/${file.id}/download` },
          });
        }
