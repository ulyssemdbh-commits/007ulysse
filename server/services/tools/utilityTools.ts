...
Pour deploy: fournir repoUrl + appName. Le système détecte auto si c'est statique ou Node.js. Pour update: juste appName (et optionnel branch). 58 actions au total.`,
            parameters: {
                ...
                properties: {
                    ...
                    isStaging: { type: "boolean", description: "Config staging (pour nginx_create, ajoute -dev au nom)" },
...
async function resolveGitHubTokenForProject(db: any, owner?: string, repo?: string, projectId?: string): Promise<string | null> {
    ...
    if (owner === 'devmaxtest' && process.env.DEVMAXTEST_GITHUB_PAT) {
        console.log(`[DevOpsGitHub] Token fallback to DEVMAXTEST_GITHUB_PAT for owner devmaxtest`);
        ...
        switch (action) {
            ...
            case "update_file": {
                ...
                if (!targetBranch) {
                    try {
                        ...
                        targetBranch = (repoInfo as any).default_branch || "main";
            ...
            case "apply_patch": {
                ...
                if (!patchBranch) {
                    try {
                        const repoInfo = await githubService.getRepo(owner, repo);
                        patchBranch = (repoInfo as any).default_branch || "main";
                    } catch { patchBranch = "main"; }
                }
                const { devopsIntelligenceEngine } = await import("../devopsIntelligenceEngine");
                const riskCheck = devopsIntelligenceEngine.autoRiskCheckForPatch(files);
                const result = await githubService.applyPatch(owner, repo, patchBranch, files, message);
                const riskEmoji = riskCheck.riskLevel === "dangerous" ? "🔴" : riskCheck.riskLevel === "risky" ? "🟠" : riskCheck.riskLevel === "caution" ? "🟡" : "🟢";
                devopsDiscordNotify("Patch appliqué", `**${message}**\n${owner}/${repo} sur \`${patchBranch}\`\n${riskEmoji} Risk: ${riskCheck.riskScore}/100 (${riskCheck.riskLevel})`, riskCheck.riskScore >= 55 ? "warning" : "success", [
                    { name: "Fichiers", value: `${files.length} fichier(s)`, inline: true },
                    { name: "Branche", value: patchBranch, inline: true },
                    { name: "Risque", value: `${riskCheck.riskScore}/100`, inline: true }
                ]);
                return JSON.stringify({
                    success: true,
                    result,
                    risk_analysis: {
                        ...
                        level: riskCheck.riskLevel,
                        warnings: riskCheck.warnings.slice(0, 5),
                        recommendations: riskCheck.recommendations.slice(0, 3),
                    }
                });
            }
            case "browse_files": {
...
export async function executeDevopsServer(args: Record<string, any>): Promise<string> {
    try {
        ...
        const { action, appName, repoUrl, branch, port, buildCmd, startCmd, envVars, domain, command, lines, createDb, dbName, dbUser, dbPassword, ssl, forceStatic, instances, cronExpression, cronCommand, backupFile, caller } = args;
        ...
        switch (action) {
            ...
            case "deploy": {
                ...
                const result = await sshService.deployApp({ repoUrl, appName, branch, port, buildCmd, startCmd, envVars, domain, createDb, dbName, dbUser, dbPassword, ssl, forceStatic, caller: deployCaller });
                if (result.success) {
                    try {
                        const ulysseUrl = `https://${domain || appName + ".ulyssepro.org"}`;
