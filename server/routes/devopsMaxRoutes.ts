...
router.delete("/contents/*", async (req: Request, res: Response) => {
  try {
    ...
    const result = await withRepoToken(repo.githubToken, () =>
      githubService.deleteFile(repo.owner, repo.name, filePath, message || `Delete ${filePath}`, branch || "main")
    );
    await logDevmaxActivity(req, "delete_file", filePath, { branch });
...
router.post("/patch", async (req: Request, res: Response) => {
  try {
    ...
    await logDevmaxActivity(req, "apply_patch", branch, { filesCount: files.length, commitMessage });
    res.json(result);
  } catch (error: any) {
    res.status(500).json({ error: error.message });
  }
