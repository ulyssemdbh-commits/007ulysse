...
import { getOllama, getGemini, getPrimaryAI, getOpenAINullable, getFallbackChain, OLLAMA_MODEL, getAIForContext, getFallbackChainForContext, type AIContext } from "../../services/core/openaiClient";
...
`;
      ...
      try {
        ...
        for (let attempt = 0; attempt < _contextFallbackChain.length; attempt++) {
          try {
            ...
            const isRetryable = retryErr.status === 429 || retryErr.status === 500 || retryErr.status === 503 || retryErr.message?.includes('Connection error') || retryErr.message?.includes('ECONNREFUSED') || retryErr.message?.includes('timed out') || retryErr.message?.includes('timeout') || retryErr.message?.includes('fetch failed') || retryErr.code === 'ETIMEDOUT' || retryErr.code === 'ESOCKETTIMEDOUT';
        ...
        if (choice.message.tool_calls && choice.message.tool_calls.length > 0) {
          ...
          while (toolRound < maxToolRounds) {
            ...
            try {
              ...
              const isRetryable = followErr.status === 429 || followErr.status === 500 || followErr.status === 503 || followErr.message?.includes('ECONNREFUSED');
        ...
        const isRetryable = err.message?.includes('timed out') || err.message?.includes('timeout') || err.code === 'ETIMEDOUT' || err.status === 429 || err.message?.includes('Connection error') || err.message?.includes('fetch failed');
    ...
    if (completion.usage) {
      ...
      metricsService.recordAICost("openai", selectedModel, completion.usage.prompt_tokens, completion.usage.completion_tokens, "chat");
    }
