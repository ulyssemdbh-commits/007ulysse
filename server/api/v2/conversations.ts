...
- Pas de listes à puces - phrases naturelles${timeContext}${webSearchContext}${generalUrlContext}${fileAttachmentContext}${agentMailContext}${curiosityContext}`;
    ...
    if (!isOwner) {
      try {
        const agentKey = isExternal ? "alfred" : "iris";
        const learningCtx = await cumulativeLearningEngine.generateLearningContext(agentKey as any, { maxInsights: 8, maxErrors: 4, includeTools: true });
        if (learningCtx) baseSystemPrompt += learningCtx;
      } catch {}
    }
...
dgm_manage: status, create_tasks, start_task, complete_task, test_task, fail_task, rollback, decompose_objective, run_pipeline, run_parallel_pipeline, get_independent_tasks, pipeline_report, next_task, clear_cache, circuit_status.
  Cycle DGM: create_tasks → start_task → [code] → complete_task → test_task → suivant. Rollback: rollback(taskId, reason) pour revert PR + redeploy. Notifications Discord auto sur chaque fin de pipeline.
...
- En fin de réponse: ce qui a été fait ✓, ce qui reste ⏳, ce qui bloque ✗.`
      ...
      let maxLearningContext = "";
      if (isMaxAI) {
        try {
          maxLearningContext = await cumulativeLearningEngine.generateLearningContext("maxai", { maxInsights: 12, maxErrors: 6, includeTools: true, domain: undefined });
        } catch {}
      }

      systemPrompt += `\n### CONTEXTE DEVOPS (interface active):\n${maxIdentity ? maxIdentity + "\n" : ""}${devopsCtx}\n${personaBlock}${chatHistoryContext}${crossSessionContext}${maxLearningContext}
${assistantModePrompt}
      ...
      try {
        ...
        if (hasDevMaxToolAccess && !hasFamilyAccess) {
          ...
          if (injectionPatterns.test(body.message || "")) {
            console.warn(`[V2-SECURITY] 🚨 Prompt injection attempt detected from tenant userId=${userId}: "${(body.message || "").slice(0, 100)}"`);
            safeWrite(`data: ${JSON.stringify({ type: "content", content: "⚠️ Ta demande contient des instructions que je ne peux pas exécuter. Je suis MaxAI, ton assistant DevOps — je t'aide à coder et déployer ton projet. Reformule ta demande en te concentrant sur le développement de ton app." })}\n\n`);
            safeWrite("data: [DONE]\n\n");
            res.end();
            return;
          }
        ...
        if (choice.message.tool_calls && choice.message.tool_calls.length > 0) {
          ...
          if (canParallelize) {
            console.log(`[V2-Tools] ⚡ PARALLEL execution of ${toolCalls.length} tool calls`);
          }

          const executeOneTool = async (toolCall: any) => {
            ...
            if (hasDevMaxToolAccess && !isOwner) {
              toolArgs._tenantContext = {
                isTenant: true,
                tenantUserId: userId,
                tenantId: body.contextHints?.devmaxTenantId || null,
                projectId: body.contextHints?.devmaxProjectId || null,
              };
            }
            
            console.log(`[V2-Tools] Executing ${toolName} via ActionHub${canParallelize ? " (parallel)" : ""}...`);
            ...
            try {
              const agentKey = (persona.toLowerCase() === "max" ? "maxai" : persona.toLowerCase()) as "ulysse" | "maxai" | "iris" | "alfred";
              cumulativeLearningEngine.recordToolCall({
                agent: agentKey,
                toolName,
                success: actionResult.success,
                durationMs: actionResult.executionMs || 0,
                error: actionResult.success ? undefined : (actionResult.error || "unknown"),
              }).catch(() => {});
            } catch (_) {}
          ...
          const toolResults = canParallelize
            ...
            : [await executeOneTool(toolCalls[0])].concat(
                await (async () => {
                  const rest: any[] = [];
                  for (let i = 1; i < toolCalls.length; i++) rest.push(await executeOneTool(toolCalls[i]));
                  return rest;
                })()
              );

          for (const tr of toolResults) {
            if (!tr.success) toolCallsSucceeded = false;
            workingMessages.push({
              role: "tool",
