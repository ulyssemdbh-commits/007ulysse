let _pLimit: any = null;
let _pRetry: any = null;
async function getPLimit() {
  if (!_pLimit) { try { const m = await import("p-limit"); _pLimit = m.default || m; } catch { _pLimit = (n: number) => { let active = 0; const queue: any[] = []; const next = () => { active--; if (queue.length > 0) queue.shift()(); }; return (fn: any) => (...args: any[]) => new Promise((resolve, reject) => { const run = () => { active++; fn(...args).then(resolve, reject).finally(next); }; active < n ? run() : queue.push(run); }); }; } }
  return _pLimit;
}
async function getPRetry() {
  if (!_pRetry) { try { const m = await import("p-retry"); _pRetry = m.default || m; } catch { _pRetry = async (fn: any, opts: any) => { for (let i = 0; i <= (opts?.retries || 3); i++) { try { return await fn(i); } catch (e) { if (i >= (opts?.retries || 3)) throw e; await new Promise(r => setTimeout(r, 1000 * Math.pow(2, i))); } } }; } }
  return _pRetry;
}
