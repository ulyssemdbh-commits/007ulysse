...
export function registerChatRoutes(app: Express): void {
  ...
  app.post("/api/conversations/:id/messages", async (req: Request, res: Response) => {
    ...
    try {
      ...
      const { content, imageDataUrl, pdfPageImages } = req.body;
      console.log(`[CHAT-DEBUG] Processing message for user ${userId}, conversation ${conversationId}${imageDataUrl ? ` with image (${(imageDataUrl.length / 1024).toFixed(1)}KB)` : ""}${pdfPageImages?.length ? ` with ${pdfPageImages.length} PDF page images` : ""}`);
      ...
      console.log(`[SPORTS-INJECT] Checking message: "${contentLower.substring(0, 60)}..."`);
      ...
      let needsSportsData = needsFootball || needsBasket || needsF1 || needsHockey || needsNFL || needsBetting;
      console.log(`[SPORTS-INJECT] Detection: football=${needsFootball}, basket=${needsBasket}, f1=${needsF1}, hockey=${needsHockey}, nfl=${needsNFL}, betting=${needsBetting}, needs=${needsSportsData}`);
...
`;
      ...
      const needsWebSearch = !isExcluded && !looksLikeCommand && !isShortMessage && !sportsDataSufficient && (
        needsRealTimeInfo ||
        hasSearchKeyword ||
