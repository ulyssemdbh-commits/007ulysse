...
import { autonomousLearningServiceV2 } from "./autonomousLearningV2";
...
import { sportsPredictionMemoryService } from "./sportsPredictionMemoryService";
import { brainSyncService } from "./brainSyncService";
import { sportsWatchService } from "./sportsWatchService";
...
import { actionHub } from "./sensory/ActionHub";
import { generateSelfReflectionJournal, formatJournalForChat } from "./selfReflectionJournal";
...
class JobScheduler {
  ...
  private registerDefaultJobs(): void {
    ...
    this.registerJob({
      ...
      execute: async () => {
        ...
        const todaysMatches = await sportsCacheService.getMatchesForDate(new Date());
        ...
        if (hour >= 5 && hour <= 7 || needsCatchUp) {
          ...
          const matchResult = await sportsCacheService.syncDailyMatches();
          console.log(`[JobScheduler] Matches synced: ${matchResult.matchesProcessed}`);
          
          // 2. Sync odds for matches
          const oddsResult = await sportsCacheService.refreshHourlyOdds();
          console.log(`[JobScheduler] Odds synced: ${oddsResult.oddsProcessed}`);
          
          // 3. Sync team stats for better predictions
          const statsResult = await sportsCacheService.syncTeamStats();
    ...
    this.registerJob({
      ...
      execute: async () => {
        ...
        if (hour === 7 || hour === 19) {
          ...
          const results = await sportsWatchService.runDailyWatch(ownerId);
    ...
    this.registerJob({
      ...
      execute: async () => {
        ...
        if (hour >= 8 && hour <= 23) {
          ...
          // Also sync multi-sport odds (NBA, NHL, NFL)
          console.log("[JobScheduler] Running multi-sport odds sync...");
          const multiResult = await sportsCacheService.syncMultiSportOdds();
    ...
    this.registerJob({
      ...
      execute: async () => {
        ...
        try {
          const result = await sportsPredictionMemoryService.trackCompletedMatches();
      ...
      execute: async () => {
        ...
        if (hour >= 2 && hour <= 4) {
          ...
          try {
            ...
            // 1. Original learning extraction
            const result = await sportsPredictionMemoryService.extractLearningsToBrain(ownerId);
    ...
    this.registerJob({
      ...
      execute: async () => {
        ...
        try {
          const results = await brainSyncService.syncAllUsers();
    ...
    this.registerJob({
      ...
      execute: async () => {
        try {
          const initiatives = await autonomousInitiativeEngine.runChecks();
          let acted = 0;
          let proposed = 0;

          for (const initiative of initiatives) {
            if (initiative.autonomyLevel === "act") {
              const result = await autonomousInitiativeEngine.executeInitiative(initiative);
              await actionHub.execute({
  ...
  private async generateMorningBrief(userId: number): Promise<{ summary: string; details: string }> {
    ...
    try {
      ...
      const matches = await sportsPredictionMemoryService.getRecentPredictions(1);
  ...
  private async generateValueBets(): Promise<Array<{ match: string; bet: string; confidence: number; value: number }>> {
    ...
    try {
      const predictions = await sportsPredictionMemoryService.getRecentPredictions(1);
      const today = new Date();
  ...
  private async generateSuguAlerts(): Promise<string[]> {
    ...
    try {
      ...
      try {
        const suguvalAlerts = await suguvalService.getLowStockAlerts();
      ...
      try {
        const maillaneAlerts = await sugumaillaneService.getLowStockAlerts();
  ...
  private async runL4AutoPromotion(): Promise<void> {
    try {
      ...
      const result = await learningOptimizerService.autoPromoteBatch(ownerId);
  ...
  private async runLearningQualityCheck(): Promise<void> {
    try {
      ...
      for (const topic of topics) {
        const result = await learningOptimizerService.accelerateLearningCycle(ownerId, topic.id);
  ...
  private async consultSuguvalWithUlysse(): Promise<void> {
    try {
      ...
      if (hour === 23 && minute === 55 && this.suguvalLastConsultDate !== todayDate) {
        ...
        const cartData = await suguvalService.getCheckedItemsForToday();
        const itemCount = cartData.length;
        
        if (itemCount > 0) {
          // Store in Ulysse's memory for the email at 23:59
          const ownerId = await getOwnerUserId();
          if (ownerId) {
    ...
    try {
      ...
      if ((isMainWindow || isCatchUpWindow) && this.suguvalLastEmailDate !== todayDate) {
        ...
        if (dayOfWeek === 0) {
          ...
          const result = await suguvalService.sendDailyEmail("Lundi");
          console.log(`[JobScheduler] Suguval email result: ${result.message}`);
        } else {
          // Lundi-Jeudi : envoi pour le lendemain
  ...
  private async consultSugumaillaneWithUlysse(): Promise<void> {
    try {
      ...
      if (hour === 23 && minute === 55 && this.sugumaillaneLastConsultDate !== todayDate) {
        ...
        const cartData = await sugumaillaneService.getCheckedItemsForToday();
        const itemCount = cartData.length;
    ...
    try {
      ...
      if ((isMainWindow || isCatchUpWindow) && this.sugumaillaneLastEmailDate !== todayDate) {
        ...
        if (dayOfWeek === 0) {
          ...
          const result = await sugumaillaneService.sendDailyEmailWithRetry("Lundi");
          console.log(`[JobScheduler] Sugumaillane email result: ${result.message}`);
        } else {
          // Lundi-Jeudi : envoi pour le lendemain
          const result = await sugumaillaneService.sendDailyEmailWithRetry();
          console.log(`[JobScheduler] Sugumaillane email result: ${result.message}`);
        }
      }
    } catch (error) {
  ...
  private async recoverFailedSuguEmails(): Promise<void> {
    try {
      ...
      if (shouldRunStartupRecovery || (hour === 6 && minute === 0 && this.suguRecoveryLastDate !== todayDate)) {
        ...
        const maillaneResult = await sugumaillaneService.recoverFailedEmails();
        ...
        try {
          const suguvalResult = await suguvalService.recoverFailedEmails();
  ...
  private async checkAndRunJobs(): Promise<void> {
    ...
    // Get owner for dynamic prioritization
    ...
    for (const [id, job] of Array.from(this.jobs.entries())) {
      ...
      // Skip low-priority jobs during off-hours
      if (skipJobs.includes(id)) {
        continue;
      }
      
      const timeSinceLastRun = now - job.lastRun;
      
      // Boost high-priority jobs - run 20% earlier
      const effectiveInterval = boostJobs.includes(id) 
        ? job.interval * 0.8 
        : job.interval;
      
      if (timeSinceLastRun >= effectiveInterval) {
        const priorityLabel = boostJobs.includes(id) ? " [BOOSTED]" : "";
        console.log(`[JobScheduler] Running job: ${job.name}${priorityLabel}`);
  ...
  private async generateWeeklyUlysseJournal(): Promise<void> {
    try {
      ...
      const journal = await generateSelfReflectionJournal(ownerId);
      
      // Save to Ulysse's Brain with max importance
      await brainService.addKnowledge(ownerId, {
        type: "reflection",
        title: `Journal d'Introspection #${journal.journalNumber}`,
        category: "self-reflection",
        content: formatJournalForChat(journal),
  ...
  private async runAutonomousLearning(): Promise<void> {
    try {
      ...
      const results = await autonomousLearningServiceV2.runLearningCycle(ownerId);
